# postsController = require '../posts/posts.controller'
# postsController.prototype.vm.title = $WPHCTaxonomies.getTitle $state.params.term, $state.params.slug

module.exports = ($log, $scope, $controller, $WPHCTaxonomies, $state) ->
    angular.extend @, $controller('WPHCPostsController', {$scope: $scope} )
    console.log @ , '@'
    vm = @
    vm.title = $WPHCTaxonomies.getTitle $state.params.term, $state.params.slug
    vm.getQuery = () ->
        console.log 'eeee'
        query = $WPHCPosts.getQuery page
    ...
    #$log.info postsController($log, $scope, $WPHCPosts) , 'WPHCTaxonomiesPostsController'
    #return postsController($log, $scope, $WPHCPosts) ;
